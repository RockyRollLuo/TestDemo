...
public class CourseController {
    ...
    public String  doSave(@ModelAttribute Course course){  //ModelAttribute模型页面和数据的绑定

        log.debug("Info of Course:");
        log.debug(ReflectionToStringBuilder.toString(course)); //强对象转成字符串输出

        //在此进行业务操作，比如数据库持久化
        course.setCourseId(123);
        return "redirect:view2/"+course.getCourseId();  //redirect重定向,forward请求转发
    }
    ...
    //用ResponseEntity方式返回json实体对象及Http状态码
